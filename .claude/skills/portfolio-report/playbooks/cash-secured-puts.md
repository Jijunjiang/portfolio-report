...
Strike selection:
1. **User preference: lower risk over yield when the two trade off.**
   Confirmed directly on SOFI — moving from $16 to $15 strike cut assignment
   probability from 34% to 25% (N(-d2)) and cost ~$135/5-contracts of
   annualized yield; the user chose the safer strike. Default toward the
   lower end of the "genuine discount" range rather than the highest-yield
   strike that still technically clears the bar — when presenting a
   candidate, prefer ~20–25% assignment probability over ~30–40% if both
   are available, and mention the safer alternative's numbers even when
   proposing the higher-yield one, the way SOFI's row now does.
2. Default to strikes below current price at a level the user would
...
3. 2–6 week expirations by default, same rationale as covered calls.
4. Same annualized-yield math as covered calls, denominated on cash secured
   rather than notional: `premium / (strike * 100) * (365 /
   days_to_expiration)`.
5. Flag earnings-window overlap the same way as covered calls.
